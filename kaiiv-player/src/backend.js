...
//
// The rest of the player asks four things of whatever is playing: play, pause,
...
// confined to this file. The page is told about the wrapper through the
// adapter's onBackend(), which is how the Moodle adapter lets the proctoring
// monitor watch a YouTube or Vimeo lesson without knowing anything about
// either.
//
// HLS is the odd one out and the cheapest: it plays in a real <video>, with the
// stream attached by whatever the page supplies — Moodle's video.js in the
// Moodle adapter, hls.js or the browser's own support anywhere else. Nothing
// downstream can tell an HLS lesson from a plain file, which is the whole
// point.
//
// Nothing here is fetched until a video needs it, and nothing is resolved by
// the bundler: the two SDKs are loaded at runtime, by URL.
/**
 * What the page can supply. Set once by index.js from the adapter.
 *
 * attachStream(element, url) -> Promise: put an HLS stream on a <video> the
 *     browser cannot play natively.
 * onBackend(backend): told about each backend once it exists.
 */
var hooks = {attachStream: null, onBackend: null};

/**
 * @param {Object} given
 */
export var configure = function(given) {
    hooks.attachStream = (given && given.attachStream) || null;
    hooks.onBackend = (given && given.onBackend) || null;
};

/**
 * A script, by URL, once.
 *
 * With an AMD loader on the page — Moodle's RequireJS — the SDK is asked for
 * through it. Vimeo ships a UMD bundle, and dropped onto such a page as a
 * plain <script> it calls define() anonymously; RequireJS rejects that with
 * "Mismatched anonymous define() module" because it never asked for it, and
 * the page then has no player and an error nothing on it explains. Asked for
 * through the loader, the define() belongs to a request it is tracking.
 *
 * Without one, a <script> tag, and the SDK puts itself on window.
 *
 * @param {String} url
 * @return {Promise<*>} what the module exported, if it went through a loader
 */
var loadScript = function(url) {
    return new Promise(function(resolve, reject) {
        var amd = window.define && window.define.amd && typeof window.require === 'function';
        if (amd) {
...
var attachStream = function(element, url) {
    ...
    // Whatever the page supplied first: it knows what it already loads.
    if (hooks.attachStream) {
        return Promise.resolve(hooks.attachStream(element, url)).then(function() {
            return element;
        });
    }

    // hls.js, if the page has it. Not bundled: most lessons are files,
    // and a customer who streams already has a player library of their
    // own choosing.
    ...
    return Promise.reject(new Error('hls_unsupported'));
};

/**
 * Load Vimeo's SDK once, and hand back its Player constructor.
 *
 * Through loadScript(), which uses the page's AMD loader when there is
...
var loadVimeo = function() {
    ...
    vimeoReady = new Promise(function(resolve, reject) {
        ...
        loadScript('https://player.vimeo.com/api/player.js').then(function(module) {
